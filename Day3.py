...
def check_conflicts(flash_sale_items, active_campaigns):
    conflicts = {}
    for campaign, items in active_campaigns.items():
        overlapping = items & flash_sale_items
        # Tìm giao giữa flash_sale_items và items của chiến dịch
        if overlapping:
            conflicts[campaign] = overlapping
            # Nếu có giao thì lưu vào dict conflicts với key là tên chiến dịch và value là set các sản phẩm bị xung đột
    has_conflict = len(conflicts) > 0 
    # Kiểm tra nếu có bất kỳ xung đột nào tồn tại trong dict conflicts
    safe_items = flash_sale_items - set().union(*active_campaigns.values()) 
    # Tìm các sản phẩm an toàn bằng cách lấy hiệu giữa flash_sale_items và hợp nhất tất cả items từ active_campaigns
